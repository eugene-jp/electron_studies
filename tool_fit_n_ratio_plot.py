...
f = lambda x, a, b, c, d, g: a*st.norm.pdf(x,b,c)+d*st.norm.pdf(x,b,g)
### the background is assumed linear
fb = lambda x, a, b: a*x+b

no,lidz=55,125
bi=lidz-no
file_d="2017_2_data.txt"
...
s_d_x=x_d 
parameters_d, err_d = opt.curve_fit(f, s_d_x, s_d_y, p0=(30000000,90,4,10000,10)) 
...
s_mc_x=x_mc 
parameters_mc, err_mc = opt.curve_fit(f, s_mc_x, s_mc_y, p0=(30000000,90,4,10000,10)) 
print("{0}\tAnd extracted the signal".format(l_wall_time(time.time()-start)))
...
ax[0].plot(x_fit,f(x_fit,*parameters_d),label="data fit with 2G, peak = {0:.2f} GeV".format(parameters_d[1])) 

ax[0].bar(s_mc_x,s_mc_y,color="k", alpha=0.1, label="signal in mc")
ax[0].plot(x_fit,f(x_fit,*parameters_mc),label="MC fit with 2G, peak = {0:.2f} GeV".format(parameters_mc[1])) 

ax[0].legend(loc='best', bbox_to_anchor=(1, 0.5))
...
plt.savefig("d_mc_combo_fit_wpu_2gauss_ratiopl_updated_data.png",bbox_inches = "tight") 
plt.show()
